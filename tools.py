...
def build_preprocessed_eeg_dataset_CNN(folder_path, duration=200):
    '''
            ...
            for file_name in file_list:
                ...
                if file_name not in skip_set:
                    ...
                    for key in all_trials_dict.keys():
                        ...
                        feature_vector_list = []
                        label_list = []
                        cur_trial = all_trials_dict[key]  # 维度为 62 * N，每400个采样点截取一个样本，不足400时舍弃
                        length = len(cur_trial[0])
                        pos = 0
                        while pos + duration <= length:
                            feature_vector_list.append(np.asarray(cur_trial[:, pos:pos + duration]))
...
def subject_dependent_data_split(feature_vector_dict, label_dict, test_set_radio=0.1):
    '''
        使用 subject_dependent 的方式做数据切分
    :param feature_vector_dict: build_preprocessed_eeg_dataset_CNN 函数返回的 feature_vector_dict
    :param label_dict: build_preprocessed_eeg_dataset_CNN 函数返回的 label_dict
    :param test_set_radio: the radio of test set
    :return train_feature, train_label, test_feature, test_label: 训练特征，训练标签，测试特征，测试标签
    '''
    train_feature = []
    train_label = []
    ...
    for experiment in feature_vector_dict.keys():
        subject = experiment.split('_')[0]
        for trial in feature_vector_dict[experiment].keys():
            num = feature_vector_dict[experiment][trial].shape[0]

            train_feature.extend(feature_vector_dict[experiment][trial][:int(num*(1-test_set_radio))])
            train_label.extend(label_dict[experiment][trial][:int(num*(1-test_set_radio))])
            test_feature.extend(feature_vector_dict[experiment][trial][int(num*(1-test_set_radio)):])
            test_label.extend(label_dict[experiment][trial][int(num*(1-test_set_radio)):])
    return train_feature, train_label, test_feature, test_label
...
class RawEEGDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return feature.permute(0,2,1).squeeze(), label
